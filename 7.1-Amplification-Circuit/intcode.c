...
// Copies saved_memory over to memory, resets instruction pointer
void reset_state(DATA* data)
{
    for (int i = 0; i < data->length; i++)
    {
        data->memory[i] = data->saved_memory[i];
    }

    data->intruction_pointer = 0;
}

int intcode(DATA* data, int input, int* output, int phase_setting)
{
    int result;
    bool phase_has_been_set = false;
